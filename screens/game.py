...
def game(isMouse=False):
    ...
    def redraw_window():
        ...
        for index in range(1, lives + 1):
            Assets.image.draw(Image.HEART_IMAGE, (config.starting_x + 37 * index - 7, 30))

        # Draw Text
        Assets.text.draw(f'{player.get_level()} / 10', sub_small_font, Colors.CYAN, (config.starting_x + 33, 75))
        ...
        Assets.text.drawSurface(score_label, (config.ending_x - score_label.get_width() - 30, 20))
        Assets.image.draw(Image.STAR_IMAGE, (config.ending_x - Image.SKULL_IMAGE.get_width() - 85 - leftScoreIdx*23, 26))
        ...
        Assets.image.draw(Image.SKULL_IMAGE, (config.ending_x - Image.SKULL_IMAGE.get_width() - 85 - leftKillsIdx*15, 82))
        kills_label = Assets.text.render(f'{kills}', sub_font, Colors.RED)
        Assets.text.drawSurface(kills_label, (config.ending_x - kills_label.get_width() - 30, 75))

        if win:
            scores.append(True, player.get_level(), player.get_score(), player.get_kills())
            Assets.text.draw('WINNER :)', pop_up_font, Colors.GREEN, (config.center_x, 350), True)
        ...
        if player.get_level() >= 10 and boss_entry:
            Assets.text.draw('BOSS LEVEL!!', pop_up_font, Colors.RED, (config.center_x, 350), True)
...
def paused(player, isMouse):
    ...
    Assets.text.drawSurface( pause_label, (config.center_x - pause_label.get_width()//2, 300))
