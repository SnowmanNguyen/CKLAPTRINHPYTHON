...
class AudioControls:
    ...
    def display_volume(self):
        ...
        if self.muted:
            Assets.image.draw(Image.MUTE_ICON, (config.starting_x+20, config.ending_y-52))
            vol_lbl_text = " --"
        else:
            Assets.image.draw(Image.VOL_ICON, (config.starting_x + 20, config.ending_y - 52))
            vol_lbl_text = str(self.volume).rjust(3, " ")

        Assets.text.draw(vol_lbl_text, control_font, Colors.WHITE, (config.starting_x + 70, config.ending_y - 57))
...
class DisplayControls:
    ...
    def toggle_full_screen(self):
        ...
        if self.fullscreen:
            ...
            config.CANVAS = pygame.display.set_mode((config.WIDTH, config.HEIGHT), pygame.RESIZABLE)
